---
类型: 建库SQL
主题: 客户管理系统 SQLite 建库语句（可直接执行）
适用对象: SQLite 3（建议开启 JSON1）
关联文档:
  - 客户管理系统数据库说明表（SQLite）
  - 客户管理系统建库SQL（SQLite）.sql
整理日期: 2026-09-14
标签: [数据库, SQLite, 建库, DDL]
---

# 客户管理系统 SQLite 建库语句

> [!info] 说明
> 1. 直接复制下方代码块保存为 `.sql`，或用 SQLite 工具/命令行执行即可建库（建议开启 JSON1 扩展）。
> 2. 本文件**仅供审阅 / 备用**，**未在环境中实际执行**。
> 3. 字段与关系说明见 [[客户管理系统数据库说明表（SQLite）]]；同内容脚本文件为 [[客户管理系统建库SQL（SQLite）.sql]]。
> 4. 脚本建表阶段自动 `PRAGMA foreign_keys = OFF`，建表完成后 `PRAGMA foreign_keys = ON`。
> 5. 共 **61 张表**、31 个索引。
